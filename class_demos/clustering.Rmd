...
```{r}
ames_dat <- recipe(Sale_Price ~., data = ames_num) %>% #need to specify outcome for preprocessing even though we won't use it for clustering
  step_dummy(all_nominal(), -all_outcomes(), one_hot = TRUE) %>% #
  step_normalize(all_numeric(), - all_outcomes(),) %>%
  prep() %>% #apply calculations for normalization. will calc what it needs to do but won't do anything. 
  bake(., ames_num) %>% #applies calculation to the data 
...
augment(ames_clust, ames_dat) %>% #allows you to take model outputs and append it to a dataframe
...
```{r}
matrix <- dist(ames_dat, method = "euclidean")
```
...
```{r}
hierarchy_clust <- hclust(matrix, method = "complete")
```
